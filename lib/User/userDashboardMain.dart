...
class UserDashboardMain extends StatefulWidget {
  @override
  State<UserDashboardMain> createState() => _UserDashboardMainState();
}
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                height: mq * 0.65,
                child: SingleChildScrollView(
                  child: StreamBuilder<List<RecruiterModel>>(
                      stream: controller.getRecruiters(),
                      builder: (context, snapshot) {
                        if (snapshot.hasData) {
                          List<RecruiterModel> filteredData = snapshot.data!
                              .where((element) => element.name
                                  .toLowerCase()
                                  .contains(_searchValue.toLowerCase()))
                              .where((element) =>
                                  selectedCompanies.isEmpty ||
                                  selectedCompanies.contains(element.company))
                              .toList();
                          return setGrid
                              ? GridView.builder(
      ...
      drawer: Drawer(
        ...
        child: ListView(
          children: [
            ...
            StreamBuilder(
                stream: controller.getCompanies(),
                builder: (context, snapshot) {
                  if (snapshot.hasData) {
