...
import 'package:mca_app_1/Admin/recruiterDetailForAdmin.dart';
...
class RecruitersList extends StatefulWidget {
  @override
  State<RecruitersList> createState() => _RecruitersListState();
}

class _RecruitersListState extends State<RecruitersList> {
  final controller = Get.put(AdminRepository());
  final authController = Get.put(AuthRepository());
  final userRepo = Get.put(RecruiterRepository());
                    ...
                    child: StreamBuilder(
                        ...
                        builder: (context, snapshotAdmin) {
                          if (snapshotAdmin.hasData) {
                            ...
                            return StreamBuilder<List<RecruiterModel>>(
                                stream: controller.getRecruiters(),
                                builder: (context, snapshot) {
                                  if (snapshot.hasData) {
                                    List<RecruiterModel> filteredData = snapshot
                                        .data!
